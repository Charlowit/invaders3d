...
import com.cescristorey.invaders3d.simulation.Ship;
...
public class Renderer {
	...
	public Renderer () {
		try {
			...
			backgroundTexture = new Texture(Gdx.files.internal("data/background.jpg"), Format.RGB565, true);
	...
	public void render (Simulation simulation, float delta) {
		...
		if (simulation.ship.lives != lastLives || simulation.score != lastScore || simulation.wave != lastWave) {
			status = "vidas: " + simulation.ship.lives + " tanda: " + simulation.wave + " puntos: " + simulation.score;
